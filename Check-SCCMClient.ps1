...
$SCCMClientLocation = "\\sccmprdcgy120.network.lan\SMS_P01\Client"
...
Function Get-WMIStatus
{
	...
	IF ($Global:InstallClient -eq "True")
	{
		...
		if((Get-WmiObject win32_ComputerSystem) -and (Get-WmiObject win32_OperatingSystem) -and (Get-WmiObject win32_Service)) {$WMIStatus = "Good"}
	}
	IF ($Global:InstallClient -ne "True")
	{
		if((Get-WmiObject win32_ComputerSystem) -and (Get-WmiObject win32_OperatingSystem) -and (Get-WmiObject win32_Service) -and (Get-WmiObject -Namespace root\ccm -Class sms_client)) {$WMIStatus = "Good"}
	}
